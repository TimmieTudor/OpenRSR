...
public class LeftHammerLargeAnim : BaseAnim
{
    ...
    public override void ResetAnimation(Vector3 newPos) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 15; j++) {
                ...
                rotZ -= rotChange;
                frames1.Add(new Frame(new Vector3(newPos.x, y, newPos.z), Quaternion.Euler(0f, 0f, rotZ), new Vector3(1f, 1f, 1f), hammerTransform.gameObject));
            }
            for (int j = 0; j < 15; j++) {
                frames1.Add(new Frame(new Vector3(newPos.x, y, newPos.z), Quaternion.Euler(0f, 0f, rotZ), new Vector3(1f, 1f, 1f), hammerTransform.gameObject));
            }
